...
<div id="app">
    <section class="login-page">
        <div class="container">
            ...
            <a id="mainPage-project-manager7" href="/manager"><i style="margin-left:10px;" class="fa fa-home"></i> الرئيسية </a>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 resize">
                        <div class="project-manager7">
                            ...
                            <nav id="sidebar6">
                                ...
                                <h4 class="detail-project-manager7">{{Session::get('consultantFirstName')}} {{Session::get('consultantLastName')}}</h4>
                                <div class="col-md-12 containersManager7">
                                    @foreach($getsMessage as $msg)
                                    @if($msg->isManager == 1)
                                    <div class="messagesManager7">
                                        <img src="{{asset('assets/images/Group 3291.png')}}" alt="Avatar" style="width:100%;">
                                        ...
                                        @if(pathinfo($msg->files, PATHINFO_EXTENSION) != 'pdf')
                                            <a href="/downloadImageConsultant/{{$msg->id}}"> <i class="firstDownMessagesManager far fa-arrow-alt-circle-down"></i> </a><span class="downloadSpan1Manager">تحميل</span> <p class="pFirstMessages"><a href="/showImageConsultant/{{$msg->id}}" target="_blank"><img class="imgRightMessages" src="{{asset('uploads/' . $msg->files)}}" width="60px" height="30px"></a> <span class="time-right newTimeRight">{{$msg->created_at}}</span></p>
                                        @else
                                            <a href="/downloadImageConsultant/{{$msg->id}}"> <i class="secondDownManager far fa-arrow-alt-circle-down"></i> </a><span class="downloadSpan2Manager">تحميل</span> <p class="pSecondMessages"><a href="/showImageConsultant/{{$msg->id}}" target="_blank"><embed class="imgRightMessages2" src="{{asset('uploads/' . $msg->files)}}" /> </a> <span class="time-right">{{$msg->created_at}}</span></p>
                                        @endif
                                        @else
                                    ...
                                    <div class="messagesManager7 darkerMessagesManager7">
                                        <img src="{{asset('assets/images/Group 3292.png')}}" alt="Avatar" class="right" style="width:100%;">
                                        @if($msg->message != null)
                                        <p>{{$msg->message}} <span class="time-right">{{$msg->created_at}}</span></p>
                                        @else
                            ...
                            <div class="row allformsInOne">
                                <div class="col-md-12 textareaMessageManager7">
                                    <form action="{{route('ProjectConsulStoreMsg')}}" method="POST">
                                        @csrf
                                        ...
                                        <input type="hidden" name="consultant_id" value="{{request()->id2}}">
                                        <input type="hidden" name="pillar_id" value="{{request()->id3}}">
                                    ...
                                    <!--    @csrf-->
                                        <img src="{{asset('assets/images/ic_link_24px.png')}}"  id="img1FileUploadMessages11" class="img1">
                                    <!--    <input type="hidden" name="project_manager_id" value="{{auth()->guard('manager')->user()->id}}" >-->
