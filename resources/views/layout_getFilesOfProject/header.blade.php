...
<div id="app">
    <section class="login-page">
        <div class="container">
            ...
            <a id="mainPage-project-manager6" href="/manager"><i style="margin-left:10px;" class="fa fa-home"></i> الرئيسية </a>
            <a id="logout-project-manager6" href="{{route('project.logout')}}">تسجيل خروج <i class="fas fa-sign-out-alt"></i></a>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-1 col-lg-1"></div>
            </nav>
            <div class="loginco">
                        ...
                        <div class="project-manager6">
                            ...
                            <nav id="sidebar5">
                                    ...
                                    <ul class="list-unstyled components" id="sidebars">
                                        ...
                                        <li class="btnProjectManager6">
                                            <button id="sendProjectToAdmin">إرسال المشروع</button>
                                        </li>
                                    </ul>
                                </nav>
                                <h4 class="detail-project-manager6">ملفات المشروع</h4>
                                <table class="table table-project-manager6">
                                    <thead>
                                        <td></td>
                                        <td width="90%">اسم الملف</td>
                                        <td width="20%">تحميل</td>
                                        <td>الحجم</td>
                                    </thead>
                                    <tbody class="filesHere">
                                        
                                            @foreach($getAllPillarsOfProject as $PillarsProject)
                                                <tr id="fahmy{{$PillarsProject->pillarsByRequest_Pillar->id}}">
                                                    <td data-toggle="collapse" data-target=".accordion{{$PillarsProject->pillarsByRequest_Pillar->id}}" class="clickable" style="cursor:pointer;"><i data-pillar="{{$PillarsProject->pillarsByRequest_Pillar->id}}" class="fa fa-caret-left arrow pillars_id"></i></td>
                                                    <td>ملفات {{$PillarsProject->pillarsByRequest_Pillar->name}}</td>
                                                    <td>تحميل الجميع</td>
                                                    <td><img data-id-request="{{request()->id2}}" data-id-pillar="{{$PillarsProject->pillarsByRequest_Pillar->id}}" onclick="downloadAllFilesPillar(this);" style="cursor:pointer;" src="{{asset('assets/images/Group 3398.png')}}"></td>
...
<script>
    $(document).ready(function(){
        ...
        var dataID = (url.match(/(\d+)/g) || []);
        var idFromUrl = dataID[1];
        var pillarID="";
        $('.fa-caret-left').on('click' , function(e){
            if ($(this).hasClass('fa-caret-left')) {
                pillarID = e.target.getAttribute('data-pillar');
                    var xhr = $.ajax({
                        url:'/getFilesFromPillars/'+pillarID+'/'+idFromUrl,
                        method:'GET',
                        success:function(data){
                            if(data != ''){
                                $.each(data , function(key , value){
                                    $(`<tr id="deleted`+pillarID+`" class="collapse in accordion`+data[key].pillar_id+`">
                                            <td><img class="img1" src="{{asset('assets/images/Group 3315.png')}}"></td>
                                            <td><p class="p1">`+data[key].files+`</p></td>
                                            <td><a href="/downloadFilesProjectManager2/`+data[key].id+`"><img class="im1" src="{{asset('assets/images/Group 3398.png')}}"></a></td>
                                            <td><p class="f1"><?php echo ceil(filesize('uploads/' . `+data[key].files+`)/1000)  . 'kB'; ?></p></td>
                                        </tr>`).insertAfter('#fahmy'+data[key].pillar_id);
                                });
                		    }else{
                		        $(`<tr class="text-center fahmico`+pillarID+`"><td></td><td>لا توجد ملفات لهذا الركن</td> <td></td> <td></td></tr>`).insertAfter('#fahmy'+pillarID);
                		    }
                        },
                    });
                }else{
                    $('.accordion'+pillarID).remove();
                    $('.fahmico'+pillarID).remove();
                }
            });
    ...
    function downloadAllFilesPillar(e){
        var request_id = e.getAttribute('data-id-request');
        var pillar_id = e.getAttribute('data-id-pillar');
        location.href="/downloadAllFilesPillarProjectManager/"+request_id+'/'+pillar_id;
    }
</script>